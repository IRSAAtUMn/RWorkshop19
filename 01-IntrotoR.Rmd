...
Note that when you first open RStudio, you will only see three windows (because you won't have any scripts or files open yet).
...
my_result <- 2 + 3^2
```
*Tip: press 'cmd+enter' on a Mac and 'cntl+enter' to create the assign arrow*
...
You can read data into R from a file on your computer, a dataset on the internet, or use data that are included in a package in R. Luckily for us, the Friday the 13th data are already stored within R in the `fivethirtyeight` package. 
...
We can take subsets of observations in the data that satisfy a criterion defined by a variable in the dataset: 

```{r, eval=FALSE}
# Subset of dates that fell on a Friday
fridays <- subset(US_births_2000_2014, US_births_2000_2014$day_of_week == "Fri")
dim(fridays)
summary(fridays)


# Subset of dates that were on the 13th of the month
thirteens <- subset(US_births_2000_2014, US_births_2000_2014$date_of_month == "13")
dim(thirteens)
summary(thirteens)


# Subset of dates that were on the 13th AND a Friday
fridays13 <- subset(US_births_2000_2014, US_births_2000_2014$day_of_week == "Fri" & US_births_2000_2014$date_of_month == "13")
dim(fridays13)
summary(fridays13)


# Subset of dates that were on the 13th OR a Friday
fridays_or_13 <- subset(US_births_2000_2014, US_births_2000_2014$day_of_week == "Fri" | US_births_2000_2014$date_of_month == "13")
dim(fridays_or_13)
summary(fridays_or_13)
...
8. Create a subset of the data that contain only movies that fail the bechdal test. Store this as `failures`. How many rows does this subset contain?  

9. Create a subset of the data that contain movies with a 2013 budget under one million dollars. Store this subset as `cheap`. How many rows does this subset contain? 
...
#8. Create a subset of the data that contain only movies that fail the bechdal test. Store this as `failures`. How many rows does this subset contain? 
failures <- subset(bechdel, bechdel$binary == "FAIL")
dim(failures)

#9. Create a subset of the data that contain movies with a 2013 budget under one million dollars. Store this subset as `cheap`. How many rows does this subset contain? 
cheap <- subset(bechdel, bechdel$budget_2013 < 1000000)
