...
<!-- take subsetting out of module 1? -->
...
2. __arrange() rows__    
    Right now, the order of the rows / candy is somewhat arbitrary.  We can `arrange()` the rows in a more meaningful way:    
...
6. `select()` columns    
    There are often more variables (columns) in a dataset than we're interested in.  Keeping these in the dataset doesn't hurt anything, but removing them can make data analysis more manageable and less overwhelming.    
    ```{r eval = FALSE}
...
7. `mutate()` columns    
    ...
    # You try: create fruity_hard variable which indicates whether candy is fruity AND hard
...
11. __Numerical summaries by group__    
    ...
    # (You try) Variability: calculate the min & max winpercent by chocolate group
...
Practice, practice, practice is important when learning the `dplyr` verbs.  Try out the following exercises with solutions below.
