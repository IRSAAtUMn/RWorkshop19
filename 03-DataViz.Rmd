...
- __Import__ the `US_births_2000_2014` data in the `fivethirtyeight package`:    
    ```{r}
    library(fivethirtyeight)
    data("US_births_2000_2014")
    ```

- __Load__ the `ggplot2` and `dplyr` packages:    
    ```{r}
    library(ggplot2)  # for visualizing data
    library(dplyr)    # for transforming data
    ```

- In our analysis, we'll focus in on births in 2014.  Use the following syntax to define this dataset (we'll discuss this syntax in the next module):    
    ```{r}
...
```{r echo=FALSE}
DT::datatable(US_births_2000_2014, options = list(pageLength = 5))
library(gridExtra)
```
...
We'll construct visualizations using the `ggplot()` function in the `ggplot2` package.  Though the `ggplot()` learning curve can be steep, its "grammar" is intuitive and generalizable once mastered.  The best way to learn about `ggplot()` is to just play around.  __Don't worry about memorizing the syntax.__  Rather, focus on the *patterns* and *potential* of their application.  There's a helpful cheat sheet for future reference:
...
We'll move onto the next section together.  If you finish early, take this time to revisit the `ggplot()` syntax.  Write out the common steps / patterns utilized in building a `ggplot`.    
...
8. **Scatterplots of 2 quantitative variables**    
...
10. __Examine the scatterplots__
...
12. __Construct side-by-side plots__    
...
13. __Interpreting the side-by-side plots__    
