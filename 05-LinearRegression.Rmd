...
Use linear regression to predict worldwide (international) gross box office sales based on domestic gross box office sales. Use the 2013 adjusted sales. Given the domestic box office gross sales for __12 Years a Slave__, predict the movie's international gross sales.

```{r}
library(fivethirtyeight)
data(bechdel)
bechdel[3,"domgross_2013"] 
#here's the domestic gross for 12 years a slave
```
...
Now, let's predict for __12 Years a Slave__. The domestic gross for this movie was 53107035. Plugging that value into the regression equation produces the predicted worldwide gross box office sales in US dollars:

```{r}
-364528.19765 + 2.09019 * 53107035
```

If you want to get a bit fancier, here are two more ways to do prediction. These are a bit more accurate because they use more decimal places.
```{r}
predict(moviemod, bechdel[3,])
moviemod$fitted.values[3]
```
